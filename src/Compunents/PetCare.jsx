...
const PetCare = ({ sortOrder , searchText}) => {
  let pets = use(petPromise);
 
   
  if (searchText) {
    pets = pets.filter((pet) =>
      pet.serviceName.toLowerCase().includes(searchText.toLowerCase())
    );
  }
   

  if (sortOrder === "asc") {
    pets = [...pets].sort((a, b) => a.rating - b.rating);
  } else if (sortOrder === "desc") {
    pets = [...pets].sort((a, b) => b.rating - a.rating);
  }
  ...
  return (
    <div className="py-10">
      ...
      <div className="grid md:grid-cols-4 gap-20  mr-8">
        {pets.map((pet) => (
          <div key={pet.serviceId} className="border border-white hover:shadow-2xl w-[270px] bg-white shadow p-3 rounded-2xl">
            <div className="space-y-3">
              <img
                src={pet.image}
                className="w-[300px] object-cover rounded-xl h-[300px]"
           ...
           {/* No results */}
        {pets.length === 0 && (
          <p className="text-xl text-gray-500">No matching services found.</p>
        )}
      </div>
    </div>
  );
